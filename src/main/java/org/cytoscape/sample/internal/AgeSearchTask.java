		    ...
		    for (int j=0; j < v_namedata.size(); j++){
		   	     ...
		   	     try {		   	     
			    	 ...
			    	 if ((inputmark) && (file.exists()) && (!updatemark)){
			    		 ...
			    		 aminoNumber = Integer.parseInt(line);
			    	...
			    	// If the local base doesn't exist
						...
						if (inputmark || updatemark){
							file.createNewFile();
							this.doubleFilePrinting(file, Integer.toString(aminoNumber), curURL);
						}					 		       	
			    	}
		 	...
		 	String[] curlines;
	       	...
	       	if (this.DImark){
		       	if (allAgesPower[i] == 0){
		       		...
		       		nodeRow.set("DI Variance", DISummx2[i] - DISumm[i]*DISumm[i]/(allAgesPower[i]-KsEmpty[i]));
		       	}
	       	}
	    }
...
//        final JFreeChart plusminuschart = createChart(dataset);
//        final ChartPanel chartPanel = new ChartPanel(plusminuschart);
...
//        
//        
//        BufferedImage plusminusimage = plusminuschart.createBufferedImage(800, 450);      
//        ImageIO.write(plusminusimage, "PNG", new File(mybasedirectory + "test.PNG"));
		...
		outTax.println(totalOrthologs);
		if (totalEdges == 0){
			outTax.println(0);
		}
		else{
			outTax.println(weightedDI/totalEdges);
		}		
		outTax.println(totalGenes);
		outTax.println(totalEdges);
		outTax.close();
		
		PrintStream out_ = new PrintStream(mybasedirectory + sep + "Output" + sep + "Pictures and reports" + sep + orgstorage.get(0) + sep + netName + sep
				+ netName + "___table___identity=" + Double.toString(equality) + "___SW-Score=" + Integer.toString(SWScore) + ".txt");
		
		out_.print("       ");
		for (int it=0; it<geneRow.size(); it++){
			out_.print(geneRow.get(it));
			out_.print("   ");
		}
		out_.println();
		for (int oit=0; oit<orgColumn.size(); oit++){
			out_.print(orgColumn.get(oit));
			for (int spcounter=0; spcounter < 10 - orgColumn.get(oit).length(); spcounter++){
				out_.print(" ");
			}
			for (int it=0; it<geneorgTable.get(oit).size(); it++){
				out_.print(geneorgTable.get(oit).get(it));
        ...
        JFreeChart chart = ChartFactory
            .createBarChart(netstorage.get(0) + " (identity=" + Double.toString(equality) +  ", SW-score=" + Integer.toString(SWScore) + ")",
        ...
        ImageIO.write(image2, "PNG", new File(deseaseDir + sep 
        		+ netName + "___graphic___identity=" + Double.toString(equality) + "___SW-Score=" + Integer.toString(SWScore) + ".PNG"));
        
        // Gene - DI table.
        if (this.DImark){
	        PrintStream outDI = new PrintStream(deseaseDir + sep 
	        		+ netName + "___DI___identity=" + Double.toString(equality) + "___SW-Score=" + Integer.toString(SWScore) + ".txt");
        ...
        // Gene - PAI table.
        PrintStream outDI = new PrintStream(deseaseDir + sep 
        		+ netName + "___PAI___identity=" + Double.toString(equality) + "___SW-Score=" + Integer.toString(SWScore) + ".txt");
...
//    }
	...
	try {  
		...
		if (outputmark){
			...
			out_ = new PrintStream(mybasedirectory + sep + "Output" + sep + "Full ages data" + sep + parsedname + ".txt");
		}
    	...
    	if ((inputmark) && (file.exists()) && (!updatemark)){
			...
			curURLagain = this.loadUrl(sURL);
	    	curlines = this.stringFounder(curURLagain, "Lineage");
	    	...
	    	if (inputmark || updatemark){
	    		file.createNewFile();
	    		this.singleFilePrinting(file, curURLagain);
	    ...
	    // Orthologs analysis. The cycle will be finished using break
   	    ...
   	    if (this.domensNumber != 0){
	   		...
	   		PrintStream outdomain = new PrintStream(mybasedirectory + sep + "Output" + sep + "OrthologDomains" + sep + tempcurOrgName + ".txt");
...
void dihappened(String curOrgName, String curHomoName, int rowCounter){
	try{
		...
		if ((inputmark) && (file.exists()) && (!updatemark)){
			...
			curURLagain = this.loadUrl(sURL);			curlines = this.stringFounder(curURLagain, "AASEQ");
			...
			if (inputmark || updatemark){
				file.createNewFile();
		   		this.doubleFilePrinting(file, organismAminoSequence, organismNucleoSequence);
		    }		}
		...
		orthologNucleoSequence = "";
			...
			if (inputmark || updatemark){
				file.createNewFile();
				this.doubleFilePrinting(file, orthologAminoSequence, orthologNucleoSequence);
			}
		}	
		...
		if (!organismAminoSequence.equals(orthologAminoSequence)){   	  	
			...
			if ((organismAminoSequence.length()>6999) || (orthologAminoSequence.length() > 6999)){
				...
				if (bigGenemark){
					...
					PrintStream logStream = new PrintStream(mybasedirectory + sep + "BigGenesLog" + sep + new java.util.Date().toString().replace(':', '_') + ".txt");
		...
		if (inputmark || updatemark){
			this.DIPrinting(difpercent, tempcurOrgName, tempcurHomoName);
			DIfile.createNewFile();
			if (lwlKs > 0){	
				this.doubleFilePrinting(DIfile, Double.toString(lwlKaKs), Double.toString(lwlKaKs*lwlKaKs));
			}	
			else{	
				this.doubleFilePrinting(DIfile, "0", "0");         	
			}	  		
	    }
	}catch (IOException e2){ e2.printStackTrace(); } 
}
...
public String[] stringFounder(String strWhereFind, String strToFind){	
	...
	while (strWhereFind.length() != 0){ //öèêë äî óïîìèíàíèÿ èñêîìîé ñòðîêè
...
public void DIPrinting(double difpercent, String orgName, String ortoName){
	try {    
