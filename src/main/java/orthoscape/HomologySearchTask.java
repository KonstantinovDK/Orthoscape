...
public class HomologySearchTask extends AbstractTask {
	...
	private CyTable nodeTable;		// network nodes table
	...
	Boolean domenmark;				// mark to use specifiñ domains
	...
	void oneurlname_parser(){
    	...
    	// If the local base doesn't exist
			...
			if (inputmark || updatemark){
				...
				OrthoscapeHelpFunctions.doubleFilePrinting(file, Integer.toString(uselessaminoNumber), curURL);
			}					 		       	
    	 }
    	// Algorithmic part
    	itishappened(curURL);     
